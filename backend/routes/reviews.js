...
router.get('/facility/:facilityId', async function (req, res) {
  try {
    ...
    var result = await reviewController.FindByFacility(req.params.facilityId, page, limit);
...
// POST /api/v1/reviews
router.post('/', checkLogin, ReviewValidator, validatedResult, async function (req, res) {
  var session = await mongoose.startSession();
  session.startTransaction();
  try {
    var { facilityId, bookingId, rating, comment } = req.body;

    var booking = await bookingController.FindById(bookingId);
    if (!booking) throw new Error('Không tìm thấy đặt sân');
    if (String(booking.user._id) !== String(req.user._id)) throw new Error('Không có quyền đánh giá');
    if (booking.status !== 'COMPLETED') throw new Error('Chỉ có thể đánh giá sau khi hoàn thành đặt sân');
    ...
    return res.status(201).json({ success: true, message: 'Đánh giá thành công', data: review });
  } catch (error) {
...
router.delete('/:id', checkLogin, async function (req, res) {
  try {
    ...
    var review = await reviewModel.findOne({ _id: req.params.id, isDeleted: false });
