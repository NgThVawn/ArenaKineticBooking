...
var facilityController = require('./facilities');
...
module.exports = {
  CreateReview: async function (data, session) {
    ...
    // Cập nhật rating trung bình của cơ sở
    await facilityController.UpdateRatingStats(data.facility);
    return review;
  },
  ...
  FindByFacility: async function (facilityId, page, limit) {
    ...
    var total = await reviewModel.countDocuments({ facility: facilityId, isDeleted: false });
  ...
  HasUserReviewed: async function (userId, facilityId) {
    var review = await reviewModel.findOne({ user: userId, facility: facilityId, isDeleted: false });
    return review !== null;
  },
